...
class WorksViewModel constructor(
    ...
    class Factory @Inject constructor(
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return WorksViewModel(repository, workRequestParams) as T
        }
    }
    ...
    init {
        val requestCreator: ((Int) -> Single<Result<List<Work>>>) = { page ->
            when (workRequestParams.type) {
                WorkRequestParams.Type.Me -> repository.me(workRequestParams.copy(page = page))
                WorkRequestParams.Type.Works -> repository.get(workRequestParams.copy(page = page))
            }
        }
